...
public class CopyOfCrawlDatum implements  Cloneable {
	...
	public static boolean hasDbStatus(CopyOfCrawlDatum datum) {
		if (datum.getStatus() <= STATUS_DB_MAX)
			return true;
		return false;
	}

	public static boolean hasFetchStatus(CopyOfCrawlDatum datum) {
		if (datum.getStatus() > STATUS_DB_MAX && datum.getStatus() <= STATUS_FETCH_MAX)
			return true;
		return false;
	}

	public CopyOfCrawlDatum() {
	}

	public CopyOfCrawlDatum(int status, int fetchInterval) {
		datum = new kim.spider.schema.CrawlDatum();
		this.setStatus(status);
		datum.fetchInterval = fetchInterval;
	}
	
	public CopyOfCrawlDatum(kim.spider.schema.CrawlDatum datum) {
		this.datum = datum; 
	}
	
	public CopyOfCrawlDatum(int status, int fetchInterval, float score) {
	...
	public void setMeta(String key,String value) {
		getMetaData().put(new Utf8(key), new Utf8(value));
	}
	
	public void setExtend(String key,String value) {
		getExtendData().put(new Utf8(key), new Utf8(value));
	}
	 ...
	 *            CrawlDatum
	 */
	public void putAllMetaData(CopyOfCrawlDatum other) {
	...
	public void set(CopyOfCrawlDatum that) {
