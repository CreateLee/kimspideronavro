...
import kim.spider.util.StringUtil;
...
public class CrawlDbReader implements Closeable {
	...
	public static class CrawlDbStatMapper implements
		...
		public void map(Text key, CrawlDatum value,
			...
			output.collect(new Text("s"),
					new LongWritable((long) (value.getScore() * 1000.0)));
			if (sort) {
				URL u = new URL(key.toString());
				String host = u.getHost();
				output.collect(new Text("status " + value.getStatus() + " "
						+ host), COUNT_1);
			}
		}
	}
	...
	public void processStatJob(String crawlDb, Configuration config,
		...
		JobConf job = new SpiderJob(config);
		...
		SequenceFile.Reader[] readers = getReaders(config, tmpFolder);
	...
	public static void main(String[] args) throws IOException {
		...
		if (args.length < 1) {
			...
			System.err
					.println("\t\t\tThis can significantly improve performance.");
			return;
		}
		String param = null;
