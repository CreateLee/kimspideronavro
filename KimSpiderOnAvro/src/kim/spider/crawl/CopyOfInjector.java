 ...
 **/
public class CopyOfInjector extends Configured implements Tool {
	public static final Log	LOG	= LogFactory.getLog(CopyOfInjector.class);

	/** Normalize and filter injected urls. */
	public static class InjectMapper extends
			Mapper<WritableComparable, Text, Utf8, kim.spider.schema.CrawlDatum> {
			...
			if (url.indexOf("\t") != -1) {
				...
				for (int s = 1; s < splits.length; s++) {
					...
					if (!metaname.equals(PARSE_CLASS)) {
						metaData.put(new Utf8(metaname), new Utf8(metavalue));
					} else
						extendData.put(new Utf8(metaname), new Utf8(metavalue));
				}
				try {
				...
				if (url != null) { // if it passes
					...
					context.write(new Utf8(url), datum.datum);
				}
			}
		}
	...
	public static class InjectReducer extends
			Reducer<Utf8, kim.spider.schema.CrawlDatum, Utf8, kim.spider.schema.CrawlDatum> {

		private CrawlDatum	old				= new CrawlDatum();
		private CrawlDatum	injected	= new CrawlDatum();

		@Override
		protected void reduce(Utf8 key, Iterable<kim.spider.schema.CrawlDatum> values, Context context)
				throws IOException, InterruptedException {
	...
	public CopyOfInjector() {
	}

	public CopyOfInjector(Configuration conf) {
		setConf(conf);
	}
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner
				.run(SpiderConfiguration.create(), new CopyOfInjector(), args);
		System.exit(res);
	}
