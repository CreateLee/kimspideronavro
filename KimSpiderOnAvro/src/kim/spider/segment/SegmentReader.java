...
import kim.spider.crawl.SpiderWritable;
...
public class SegmentReader extends Configured implements
		Reducer<Text, SpiderWritable, Text, Text> {
	...
	public static class InputCompatMapper extends MapReduceBase implements
			Mapper<WritableComparable, Writable, Text, SpiderWritable> {
		private Text newKey = new Text();

		public void map(WritableComparable key, Writable value,
				OutputCollector<Text, SpiderWritable> collector, Reporter reporter)
				throws IOException {
			...
			collector.collect((Text) key, new SpiderWritable(value));
		}
	...
	public static class TextOutputFormat extends
		...
		public RecordWriter<WritableComparable, Writable> getRecordWriter(
			...
			return new RecordWriter<WritableComparable, Writable>() {
				public synchronized void write(WritableComparable key,
	...
	public SegmentReader(Configuration conf, boolean co, boolean fe,
			boolean ge, boolean pa, boolean pd, boolean pt) {
		super(conf);
		this.co = co;
		this.fe = fe;
		this.ge = ge;
	...
	public void reduce(Text key, Iterator<SpiderWritable> values,
	...
	public void dump(Path segment, Path output) throws IOException {
		...
		job.setOutputValueClass(SpiderWritable.class);
				...
				for (int i = 0; i < files.length; i++) {
					...
					try {
						...
						if (LOG.isWarnEnabled()) {
							LOG.warn("Couldn't copy the content of "
									+ partFile.toString() + " into "
									+ dumpFile.toString());
		...
		if (pd)
			threads.add(new Thread() {
				public void run() {
					try {
						List<Writable> res = getMapRecords(new Path(segment,
								Spider.PARSE_DATA_DIR_NAME), key);
						results.put("pd", res);
					} catch (Exception e) {
						e.printStackTrace(LogUtil.getWarnStream(LOG));
					}
	...
	public void getStats(Path segment, final SegmentReaderStats stats)
		...
		Path parseDir = new Path(segment, Spider.PARSE_DATA_DIR_NAME);
		if (fs.exists(fetchDir) && fs.getFileStatus(fetchDir).isDir()) {
			cnt = 0L;
			long errors = 0L;
	...
	public static void main(String[] args) throws Exception {
		...
		case MODE_GET:
			...
			segmentReader.get(new Path(input), new Text(key),
					new OutputStreamWriter(System.out, "UTF-8"),
