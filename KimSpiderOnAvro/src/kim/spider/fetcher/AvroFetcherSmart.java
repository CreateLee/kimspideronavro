...
public class AvroFetcherSmart extends Configured implements Tool {
	...
	public static class FetchMapper extends
			BlockMapper<String, kim.spider.schema.CrawlDatum, String, Object> {
		...
		private void output(String key, CrawlDatum datum, Content content,
			...
			try {
				outer.write(key, new GenericAvroData(outer.getConfiguration(),datum.datum));

				if (content != null)
					outer.write(key, new GenericAvroData(outer.getConfiguration(),content));
	...
	public void fetch(Path segment, int threads) throws IOException,
		...
		job.setMapOutputValueClass(GenericAvroData.class);
		...
		MultipleOutputs.addNamedOutput(job, CONTENT_REDIR,
				AvroMapOutputFormat.class, String.class, kim.spider.schema.Content.class);
	...
	public static class MOReduce extends Reducer<String, GenericAvroData, String, Object> {
		...
		@Override
		protected void reduce(String key, Iterable<GenericAvroData> values, Context context)
				throws IOException, InterruptedException {
